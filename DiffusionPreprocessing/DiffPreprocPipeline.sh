...
runcmd="" ##RUNCMD DOSENT EXIST OTHER AREAS?
...
opts_AddOptional '--combine-data-flag' 'CombineDataFlag' "Specified value is passed as the CombineDataFlag value for the eddy_postproc.sh script. If JAC resampling has been used in eddy, this value determines what to do with the output file.
...
##DO WE NEED THE OPTION BELOW?
# opts_AddOptional '--printcom' 'Printcom' 'to echo or otherwise  output the commands that would be executed instead of  actually running them. --printcom=echo is intended to  be used for testing purposes>     ' "print-command>=<Use the specified <print-command"
...
##########THESE ARE MANDITORY BUT ALSO HAVE DEFAULTS SO THEY SHOULD BE OPTIONAL RIGHT? 
# if [ -z ${b0maxbval} ]; then
# 	error_msgs+="\nERROR: <b0-max-bval> not specified"
# fi
